...
This compiles the client into a jar located at `./target/CelebritiesUrl-1.0-SNAPSHOT.jar`. Note that this jar does not include any dependencies.
...
```perl
dependencies {
    compile 'com.mobimore:CelebritiesUrl:1.0-SNAPSHOT'
}
```
...
<dependencies>
    <dependency>
        <groupId>com.mobimore</groupId>
        <artifactId>CelebritiesUrl</artifactId>
...
This will compile the client and package all dependencies into a jar located at `./target/CelebritiesUrl-1.0-SNAPSHOT.jar`.
...
```java
CelebritiesUrlClientBuilder builder = CelebritiesUrl.builder();
```
...
```java
CelebritiesUrl client = CelebritiesUrl.builder()
...
## Making requests
After a client is configured and created, you can make a request to the service. A method on the client interface (`CelebritiesUrl`) is created for all actions (resource + method) in your API.
...
```java
CelebritiesUrl client = CelebritiesUrl.builder().build();
GetCelebrityUrlsResult result = client.getCelebrityUrls(new GetCelebrityUrlsRequest());
```
...
```java
CelebritiesUrl client = CelebritiesUrl.builder().build();
client.getCelebrityUrls(new GetCelebrityUrlsRequest().sdkRequestConfig(
...
Service exceptions and client exceptions can be handled separately. Any exceptions modeled in the API will be a subtype of CelebritiesUrlException.
```java
try {
    client.getCelebrityUrls(...);
} catch(CelebritiesUrlException e) {
   // All service exceptions will extend from CelebritiesUrlException.
   // Any unknown or unmodeled service exceptions will be represented as a CelebritiesUrlException.
